...
function CourseDetailList() {
    ...
    const confirmDelete = async () => {
        try{
            await $http.deleteData(CONFIG.API_BASE_URL +'/baiviet/'+idDelete.id);
            enqueueSnackbar('Xóa thành công', { variant: 'success', autoHideDuration: 3000});
            handleCloseDelete();
            setLoad(e=> e+1)
        }catch (e) {
            console.log(e)
                                    ...
                                    {e?.tailieus.map((e1) => (
                                        <div key={e1.id} className="bg-cyan-100 p-3 rounded-md">
                                            <Link to={"/app/document/detail/" + e1.id}>
                                                <div
                                                    className="mb-4 w-full h-[160px] bg-slate-200 rounded-lg overflow-hidden"/>
                                                <p>
