...
function DocumentDetail() {
  ...
  useEffect(()=>{
    (async ()=>{
      ...
      if(
        ...
        const ct = await $http.getData(CONFIG.API_BASE_URL + '/tailieu/file/'+res.data?.path);
        setRaw(ct.data);
      }
    })()
  },[]);
  ...
  return (
    <div>
     <div>
      <span className='text-base font-bold	'>Tên tài liệu: </span> <span>{data?.name}</span>
     </div>
     <div>
    ...
    <div className="flex flex-wrap space-x-2 items-start">
       ...
       <span key={idx} className="px-4 py-2 my-2 rounded-full text-white bg-orange-400 font-semibold flex align-center w-max cursor-pointer active:bg-gray-300 transition duration-300 ease">
       {e?.name}
       </span>
      ))} 
    </div>
     
      {raw &&<> <div className="flex items-center gap-2 cursor-pointer" onClick={()=>{window.location.href = CONFIG.API_BASE_URL+'/documents/'+data?.path}}>
          <FaDownload className="cursor-pointer" size={25} />
          <p>Tải về</p>     
      </div> <Code code={raw} language="javascript" /> </>}
      {language === 'pdf' && 
      <div className="mt-5 w-[90%] h-[100vh] bg-slate-400">
         <iframe src={CONFIG.API_BASE_URL+'/documents/'+data?.path} width='100%' height='100%' />
      </div>}
      {(language !== 'pdf' && !raw) &&
