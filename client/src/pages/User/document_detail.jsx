...
function DocumentDetail() {
    ...
    return (
        <div>
            ...
            <div>
                <span className="font-bold">Tạo bởi: </span> <Link
                to={`/app/user/${data?.user?.maso}`}><span>{data?.user?.ho_ten}</span></Link>
            </div>
            ...
            {language !== "pdf" && !raw && (
                <div className="bg-white w-[60%] p-3 rounded-md">
                    ...
                    <div className="flex justify-end items-center gap-5">
                        {user?.maso !== data?.user?.maso && <div
                            className="flex items-center gap-2 p-2 cursor-pointer bg-yellow-300 rounded-md shadow-md"
                        >

                            {
                                !checkTL(data.id, user.tl) ? <MdOutlineDataSaverOn
                                    color="#2979ff"
                                    className="ml-auto mr-0 cursor-pointer"
                                    onClick={async () => dispatch(await fnSaveTaiLieu(user.id, data.id, user.maso))}
                                /> : <AiOutlineDisconnect
                                    color="#2979ff"
                                    onClick={async () => dispatch(await fnUnSaveTaiLieu(user.id, data.id, user.maso))}
                                    className="ml-auto mr-0 cursor-pointer"/>
                            }
                        </div>}
                    </div>
