...
function DocumentUpload() {
  ...
  const setActiveTL = async (id, active) => {
    try {
      await $http.postData(CONFIG.API_BASE_URL + "/tailieu/active", {
        id: id,
        active: active,
      });
  ...
  return (
    <div>
      ...
      >
        {t("title.add_document")} <HiOutlineDocumentAdd size={25} />
      </button>
      ...
      >
        <Box sx={style}>
          <p className="text-center text-[25px] font-medium">
            {t("title.add_document")}
          </p>
          <form onSubmit={handleSubmit(onSubmit)}>
            <p>
              <strong>{t("title.document_name")}</strong>
            </p>
            <div className="relative mt-2 mb-4">
              <input
                type="text"
                className=" py-1 px-4 w-full border border-[#ccc] rounded-md"
                placeholder={`${t("title.enter_name_document")}`}
                name="tentailieu"
                {...register("tentailieu")}
              />
            ...
            <div className="mt-6 mb-2">
              <p>
                <strong>{t("title.describe")}</strong>
              </p>
              <textarea
                type="text"
                rows={4}
                className=" py-1 px-4 w-full border border-[#ccc] rounded-md"
                placeholder={`${t("title.enter_describe")}`}
                name="mota"
                {...register("mota")}
              />
            </div>
            <div className="flex flex-wrap justify-center space-x-2">
              ...
              <span>{LV.length === 0 && `${t("title.enter_fields")}`}</span>
            </div>
            <div className="flex flex-wrap justify-center space-x-2">
              {lv?.map((e, idx) => (
                <span
            ...
            <p>
              <strong>{t("title.select_file")}</strong>
            </p>
            <input
              type="file"
              className="my-2"
      ...
      <Modal open={openSetactive} onClose={() => setOpenSetactive(false)}>
        <Box sx={style}>
          <div className="text-center text-[25px] font-medium">
            Bạn chắc chắn muốn xóa tài liệu
          </div>
          <div className="flex gap-2 justify-end my-4">
            <button
              className="py-2 px-4 min-w-[100px] text-white font-bold bg-[#F38E46] rounded-md shadow-md hover:opacity-90 duration-300"
